...
async function deleteCriterion(id: string): Promise<void> {
  const res = await fetch(`${API}/api/criteria/${id}`, { method: 'DELETE', credentials: 'include' })
...
export default function CriteriaManager() {
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', padding: 24 }}>
      ...
      <form onSubmit={handleSubmit} style={{ background: '#f9fafb', borderRadius: 8, padding: 16, marginBottom: 24 }}>
        ...
        <button type="submit" disabled={createMutation.isPending}
          style={{ marginTop: 12, background: '#6366f1', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 20px', cursor: 'pointer' }}>
          {createMutation.isPending ? 'Adding…' : 'Add Criterion'}
        </button>
      </form>

      {criteria.length === 0 && <p style={{ color: '#6b7280' }}>No custom criteria yet.</p>}

      {criteria.map((c) => (
        <div key={c.id} style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: 12, marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
