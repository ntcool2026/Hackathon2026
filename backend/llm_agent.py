...
# Fallback model chain — tried in order on 429 or function-calling failure.
# Deliberately diverse providers so a rate limit on one doesn't block all.
_MODEL_FALLBACKS = [
...
def _sanitize_rationale(rationale: str | None, ticker: str, score: float, rec: str | None) -> str:
    """Return a clean rationale, replacing leaked/empty content with a score-derived fallback."""
    if not rationale or _looks_like_leaked_prompt(rationale) or len(rationale.strip()) < 15:
        rec_str = rec or "HOLD"
        if score < 35:
...
async def _call_openrouter_with_tools(
    ...
    Implements the model fallback chain. When tool_schemas are provided,
    skips models that return plain text (no tool_calls) — treating them as
    ...
    for model in models:
        ...
        try:
            ...
            if resp.status_code == 429:
                ...
                logger.warning("429 on %s, trying next model", model)
            ...
            # If tools were offered but the model returned plain text with no tool_calls
            # and no structured output, it likely doesn't support function calling — skip it.
            if tool_schemas and not msg.get("tool_calls"):
                content = (msg.get("content") or "").strip()
                if content and "AI_RISK_SCORE" not in content.upper():
                    logger.debug("Model %s returned plain text without tool_calls, trying next", model)
                    last_error = RuntimeError(f"Model {model} does not support function calling")
                    continue
            ...
            last_error = exc
            logger.warning("HTTP error from %s (%s), trying next", model, exc.response.status_code)
        except Exception as exc:
            last_error = exc
            logger.warning("Error calling %s (%s), trying next", model, exc)
...
async def _run_tool_call_phase(
    ...
    while tool_call_count < max_calls:
        data = await _call_openrouter_with_tools(messages, tool_schemas)
        choice = data["choices"][0]
        msg = choice.get("message", {})
        ...
        for tc in tool_calls:
            ...
            if adapter is None:
                ...
                try:
                    ...
                    logger.warning("Tool call %s failed for %s: %s", tool_name, ticker, exc)
...
async def _run_reflection_loop(
    ...
    for round_num in range(max_rounds):
        ...
        try:
            ...
            text = content if "AI_RISK_SCORE" in content.upper() else (reasoning if "AI_RISK_SCORE" in reasoning.upper() else content)

            new_score, new_rec, new_rationale = _parse_structured_output(text)

            if new_score is None:
                logger.warning("Reflection round %d parse failure for score — stopping reflection", round_num + 1)
                break

            messages.append({"role": "assistant", "content": content})

            delta = abs(new_score - score)
            score, rec, rationale = new_score, new_rec or rec, new_rationale or rationale

            logger.info("Reflection round %d: score=%.1f delta=%.1f", round_num + 1, score, delta)

            if delta < delta_threshold:
                logger.info("Reflection stopping early (delta %.1f < threshold %.1f)", delta, delta_threshold)
                break

        except Exception as exc:
            logger.warning("Reflection round %d failed: %s — stopping", round_num + 1, exc)
            break
...
def _parse_structured_output(raw: str) -> tuple[float | None, str | None, str | None]:
    ...
    score_match = re.search(r'AI_RISK_SCORE\s*:\s*(-?[0-9]+(?:\.[0-9]+)?)', raw, re.IGNORECASE)
    rec_match = re.search(r'AI_RECOMMENDATION\s*:\s*(BUY|HOLD|SELL)', raw, re.IGNORECASE)
    ...
    if ai_rec is None:
        ...
        if fb_rec:
            ...
            counts = {k: len(re.findall(rf'\b{k}\b', raw, re.IGNORECASE)) for k in ('BUY', 'HOLD', 'SELL')}
    ...
    if rationale is None:
        ...
        rationale = ' '.join(sentences[-3:]).strip() if len(sentences) >= 3 else raw.strip()
...
async def _prefetch_tool_data(ticker: str, adapter_map: dict) -> dict[str, Any]:
    ...
    results = await asyncio.gather(*[_fetch_one(name, adapter) for name, adapter in adapter_map.items()])
    return {name: data for name, data in results if data is not None}


def _build_analysis_user_message(ticker: str, risk_score: float, tool_data: dict[str, Any] | None = None) -> str:
...
async def _generate_rationale(
    ...
    try:
        ...
        if tool_data:
            logger.info("Prefetched %d tool(s) for %s: %s", tool_call_count, ticker, list(tool_data.keys()))
        else:
            logger.info("No tool data available for %s — using quant score only", ticker)
        ...
        if last_assistant and "AI_RISK_SCORE" in last_assistant.upper():
            ...
            content = _extract_structured_block((msg.get("content") or "").strip())
            reasoning = _extract_structured_block((msg.get("reasoning") or "").strip())
            raw_analysis = content if "AI_RISK_SCORE" in content.upper() else (reasoning if "AI_RISK_SCORE" in reasoning.upper() else content)
            messages.append({"role": "assistant", "content": raw_analysis})

        initial_score, initial_rec, initial_rationale = _parse_structured_output(raw_analysis)

        if initial_score is None:
            logger.warning("Initial parse failed for %s — using quant score as fallback", ticker)
        ...
        initial_rationale = _sanitize_rationale(initial_rationale, ticker, initial_score, initial_rec)
        ...
        final_rationale = _sanitize_rationale(final_rationale, ticker, final_score, final_rec)

        logger.info(
            "LLM agent loop complete for %s: tool_calls=%d reflection_rounds=%d ai_score=%.1f",
            ticker, tool_call_count, settings.llm_max_reflection_rounds, final_score,
        )
...
async def run_llm_agent_cycle() -> None:
    ...
    try:
        ...
        logger.warning("Failed to build tool registry for LLM cycle: %s — proceeding without tools", exc)
        tool_schemas = []
        adapter_map = {}
    ...
    tasks = [
        _process_ticker_for_users(ticker, user_scores, semaphore, tool_schemas, adapter_map)
...
async def _process_ticker_for_users(
    ...
    for user_id, _ in user_scores:
        async with AsyncSessionLocal() as db:
            await _persist_rationale(db, user_id, ticker, rationale, ai_risk_score, ai_recommendation)
...
async def run_portfolio_analysis(user_id: str, ticker_data: list[dict]) -> None:
    ...
    try:
        ...
        logger.warning("Portfolio analysis LLM call failed for user %s: %s", user_id, exc)
        summary = f"Portfolio contains {total} holdings across {len(sector_counts)} sectors."

    result = PortfolioAnalysisResult(summary=summary, concentration_flags=concentration_flags)
    ...
    await ws_manager.broadcast_to_user(
        ...
        WSEvent(
            ...
            payload={"summary": result.summary, "concentration_flags": result.concentration_flags},
        ),
    )
        ...
        async with AsyncSessionLocal() as db:
            ...
            for ticker, score in tickers_scores:
                ...
                ticker_data.append({
                    ...
                    "ai_risk_score": float(score_row.ai_risk_score) if score_row and score_row.ai_risk_score else score,
                    "ai_recommendation": score_row.ai_recommendation if score_row else None,
                })
...
async def _get_candidates(db) -> list[tuple[str, str, float, float]]:
    ...
    for row in rows:
        ...
        if row.ai_risk_score is None:
            ...
            rationale_age = now - row.rationale_at if row.rationale_at.tzinfo is not None else now - row.rationale_at.replace(tzinfo=timezone.utc)
...
async def _persist_rationale(
    ...
    rationale: str, ai_risk_score: float | None, ai_recommendation: str | None,
) -> None:
    now = datetime.now(tz=timezone.utc)
    ...
    params: dict = {"r": rationale, "ai": ai_risk_score, "airec": ai_recommendation, "uid": user_id, "ticker": ticker}
    if rationale_at_value is not None:
        set_clause += ", rationale_at = :t"
        params["t"] = rationale_at_value

    await db.execute(
        text(f"UPDATE stock_scores SET {set_clause} WHERE user_id = :uid AND ticker = :ticker"),
        params,
    )
    await db.commit()
