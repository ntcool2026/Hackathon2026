...
from civic_auth.integrations.fastapi import create_auth_router, FastAPICookieStorage
from civic_auth.storage import AuthStorage, CookieStorage
...
# ---------------------------------------------------------------------------
# In-memory storage — used during callback to capture tokens reliably
# ---------------------------------------------------------------------------

class MemoryStorage(AuthStorage):
    """Simple dict-backed storage for capturing tokens during OAuth callback."""

    def __init__(self, seed: dict | None = None) -> None:
        self._data: dict[str, str] = dict(seed or {})
...
# ---------------------------------------------------------------------------
# Hybrid storage: Bearer header → cookie fallback (for API requests)
# ---------------------------------------------------------------------------
...
# Re-register civic routes except /auth/callback and /auth/user (we override both)
...
async def get_user_endpoint(request: Request, response: Response):
    """Get current user — supports both cookie and Bearer token auth."""
    civic = await civic_auth_dep(request, response)
    user = await civic.get_user()
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not authenticated")
...
async def auth_callback(code: str, state: str, request: Request):
    """OAuth callback: exchange code for tokens, redirect to frontend with token in URL fragment."""
    # Seed memory storage with existing cookies (needed for PKCE verifier + state)
    seed = {k: v for k, v in request.cookies.items()}
    mem_storage = MemoryStorage(seed)
    civic = CivicAuth(mem_storage, _config)
    ...
    # Token is now reliably in memory storage
    id_token = await mem_storage.get(CivicAuth.ID_TOKEN_KEY)
    logger.info("auth_callback: id_token present=%s", bool(id_token))
    ...
    # Also set cookies for local dev cookie-based flow
    if id_token:
        redirect_response.set_cookie(
            key=CivicAuth.ID_TOKEN_KEY, value=id_token,
            httponly=True, samesite="lax", path="/"
        )
    access_token = await mem_storage.get(CivicAuth.ACCESS_TOKEN_KEY)
    if access_token:
        redirect_response.set_cookie(
            key=CivicAuth.ACCESS_TOKEN_KEY, value=access_token,
            httponly=True, samesite="lax", path="/"
        )
...
async def get_token(request: Request):
    """Return the Civic id_token from cookie (local dev helper)."""
